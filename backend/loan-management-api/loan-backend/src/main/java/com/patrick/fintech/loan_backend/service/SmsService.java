...
public class SmsService {
    ...
    @Async public void sendLoanApproved(Loan loan) {
        String phone=phone(loan); if(phone==null)return;
    ...
    @Async public void sendLoanRejected(Loan loan) {
        String phone=phone(loan); if(phone==null)return;
    ...
    @Async public void sendLoanDisbursed(Loan loan, String method) {
        String phone=phone(loan); if(phone==null)return;
    ...
    @Async public void sendPaymentDue(Loan loan,double amount,String dueDate) {
        String phone=phone(loan); if(phone==null)return;
    ...
    @Async public void sendPaymentConfirmed(Loan loan,double amount) {
        String phone=phone(loan); if(phone==null)return;
    ...
    @Async public void sendLoanOverdue(Loan loan,int days) {
        String phone=phone(loan); if(phone==null)return;
    ...
    @Async public void sendCustom(String phone,String message) { send(phone,message); }

    private void send(String to,String msg) {
        if(!smsEnabled){log.info("[SMS] {} -> {}",to,msg);return;}
    ...
    private boolean trySendAT(String to,String msg) {
        ...
        try {
            ...
            h.set("apiKey",atApiKey); h.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
            ...
            log.debug("SMS sent via Africa's Talking to {}",to); return true;
        } catch(Exception e){log.warn("AT SMS failed: {}",e.getMessage());return false;}
    }
    private boolean trySendTwilio(String to,String msg) {
        ...
        try {
            ...
            HttpHeaders h=new HttpHeaders();
            h.setBasicAuth(twilioAccountSid,twilioAuthToken); h.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
            ...
            log.debug("SMS sent via Twilio to {}",to); return true;
        } catch(Exception e){log.warn("Twilio SMS failed: {}",e.getMessage());return false;}
    }
    private String enc(String s){try{return java.net.URLEncoder.encode(s,"UTF-8");}catch(Exception e){return s;}}
    private String fmt(Double v){return v==null?"0":String.format("%,.2f",v);}
    private String phone(Loan l){return(l.getBorrower()!=null&&l.getBorrower().getPhone()!=null&&!l.getBorrower().getPhone().isBlank())?l.getBorrower().getPhone():null;}
