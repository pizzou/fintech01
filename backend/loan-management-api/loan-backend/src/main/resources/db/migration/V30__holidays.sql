-- V30: Holiday.java and HolidayRepository were added (for computing business-day loan/payment
...
-- Hibernate's schema-validation on startup (spring.jpa.hibernate.ddl-auto=validate) correctly
-- rejected this: the "holidays" table doesn't exist at all, so entityManagerFactory failed to
-- initialize, which cascaded through UserRepository -> CustomUserDetailsService -> JwtAuthFilter
-- (every repository bean lives in the same persistence unit, so one bad entity blocks all of them).
-- This migration creates the table the entity has expected all along.
